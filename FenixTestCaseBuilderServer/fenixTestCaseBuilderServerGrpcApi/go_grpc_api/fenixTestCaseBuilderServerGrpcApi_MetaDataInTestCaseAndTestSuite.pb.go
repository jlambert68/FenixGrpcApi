// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.31.0
// 	protoc        v3.12.4
// source: FenixTestCaseBuilderServer/fenixTestCaseBuilderServerGrpcApi/fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite.proto_old

package go_grpc_api

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// MetaDataSelectTypeEnum
// The type used for SelectType
type MetaDataSelectTypeEnum int32

const (
	MetaDataSelectTypeEnum_MetaDataSelectType_NotSelected  MetaDataSelectTypeEnum = 0
	MetaDataSelectTypeEnum_MetaDataSelectType_SingleSelect MetaDataSelectTypeEnum = 1
	MetaDataSelectTypeEnum_MetaDataSelectType_MultiSelect  MetaDataSelectTypeEnum = 2
)

// Enum value maps for MetaDataSelectTypeEnum.
var (
	MetaDataSelectTypeEnum_name = map[int32]string{
		0: "MetaDataSelectType_NotSelected",
		1: "MetaDataSelectType_SingleSelect",
		2: "MetaDataSelectType_MultiSelect",
	}
	MetaDataSelectTypeEnum_value = map[string]int32{
		"MetaDataSelectType_NotSelected":  0,
		"MetaDataSelectType_SingleSelect": 1,
		"MetaDataSelectType_MultiSelect":  2,
	}
)

func (x MetaDataSelectTypeEnum) Enum() *MetaDataSelectTypeEnum {
	p := new(MetaDataSelectTypeEnum)
	*p = x
	return p
}

func (x MetaDataSelectTypeEnum) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (MetaDataSelectTypeEnum) Descriptor() protoreflect.EnumDescriptor {
	return file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_enumTypes[0].Descriptor()
}

func (MetaDataSelectTypeEnum) Type() protoreflect.EnumType {
	return &file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_enumTypes[0]
}

func (x MetaDataSelectTypeEnum) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use MetaDataSelectTypeEnum.Descriptor instead.
func (MetaDataSelectTypeEnum) EnumDescriptor() ([]byte, []int) {
	return file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_rawDescGZIP(), []int{0}
}

// UserSpecifiedTestCaseMetaDataMessage
// The users specified values for the TestCase MedataData
type UserSpecifiedTestCaseMetaDataMessage struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	CurrentSelectedDomainUuid string                           `protobuf:"bytes,1,opt,name=CurrentSelectedDomainUuid,proto3" json:"CurrentSelectedDomainUuid,omitempty"`                                                                         // Specifies the current selected Owner Domain, Uuid, for the TestCase
	CurrentSelectedDomainName string                           `protobuf:"bytes,2,opt,name=CurrentSelectedDomainName,proto3" json:"CurrentSelectedDomainName,omitempty"`                                                                         // Specifies the current selected Owner Domain, Name, for the TestCase
	MetaDataGroupsMap         map[string]*MetaDataGroupMessage `protobuf:"bytes,3,rep,name=MetaDataGroupsMap,proto3" json:"MetaDataGroupsMap,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"` // holding MetaDataGroups and their MetaData. The key is the MetaDataGroupName
}

func (x *UserSpecifiedTestCaseMetaDataMessage) Reset() {
	*x = UserSpecifiedTestCaseMetaDataMessage{}
	if protoimpl.UnsafeEnabled {
		mi := &file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UserSpecifiedTestCaseMetaDataMessage) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UserSpecifiedTestCaseMetaDataMessage) ProtoMessage() {}

func (x *UserSpecifiedTestCaseMetaDataMessage) ProtoReflect() protoreflect.Message {
	mi := &file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UserSpecifiedTestCaseMetaDataMessage.ProtoReflect.Descriptor instead.
func (*UserSpecifiedTestCaseMetaDataMessage) Descriptor() ([]byte, []int) {
	return file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_rawDescGZIP(), []int{0}
}

func (x *UserSpecifiedTestCaseMetaDataMessage) GetCurrentSelectedDomainUuid() string {
	if x != nil {
		return x.CurrentSelectedDomainUuid
	}
	return ""
}

func (x *UserSpecifiedTestCaseMetaDataMessage) GetCurrentSelectedDomainName() string {
	if x != nil {
		return x.CurrentSelectedDomainName
	}
	return ""
}

func (x *UserSpecifiedTestCaseMetaDataMessage) GetMetaDataGroupsMap() map[string]*MetaDataGroupMessage {
	if x != nil {
		return x.MetaDataGroupsMap
	}
	return nil
}

// UserSpecifiedTestSuiteMetaDataMessage
// The users specified values for the TestSuite MedataData
type UserSpecifiedTestSuiteMetaDataMessage struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	CurrentSelectedDomainUuid string                           `protobuf:"bytes,1,opt,name=CurrentSelectedDomainUuid,proto3" json:"CurrentSelectedDomainUuid,omitempty"`                                                                         // Specifies the current selected Owner Domain, Uuid, for the TestSuite
	CurrentSelectedDomainName string                           `protobuf:"bytes,2,opt,name=CurrentSelectedDomainName,proto3" json:"CurrentSelectedDomainName,omitempty"`                                                                         // Specifies the current selected Owner Domain, Name, for the TestSuite
	MetaDataGroupsMap         map[string]*MetaDataGroupMessage `protobuf:"bytes,3,rep,name=MetaDataGroupsMap,proto3" json:"MetaDataGroupsMap,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"` // holding MetaDataGroups and their MetaData. The key is the MetaDataGroupName
}

func (x *UserSpecifiedTestSuiteMetaDataMessage) Reset() {
	*x = UserSpecifiedTestSuiteMetaDataMessage{}
	if protoimpl.UnsafeEnabled {
		mi := &file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UserSpecifiedTestSuiteMetaDataMessage) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UserSpecifiedTestSuiteMetaDataMessage) ProtoMessage() {}

func (x *UserSpecifiedTestSuiteMetaDataMessage) ProtoReflect() protoreflect.Message {
	mi := &file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UserSpecifiedTestSuiteMetaDataMessage.ProtoReflect.Descriptor instead.
func (*UserSpecifiedTestSuiteMetaDataMessage) Descriptor() ([]byte, []int) {
	return file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_rawDescGZIP(), []int{1}
}

func (x *UserSpecifiedTestSuiteMetaDataMessage) GetCurrentSelectedDomainUuid() string {
	if x != nil {
		return x.CurrentSelectedDomainUuid
	}
	return ""
}

func (x *UserSpecifiedTestSuiteMetaDataMessage) GetCurrentSelectedDomainName() string {
	if x != nil {
		return x.CurrentSelectedDomainName
	}
	return ""
}

func (x *UserSpecifiedTestSuiteMetaDataMessage) GetMetaDataGroupsMap() map[string]*MetaDataGroupMessage {
	if x != nil {
		return x.MetaDataGroupsMap
	}
	return nil
}

// MetaDataGroupMessage
// Message holding alla a Group and all of its Items with their values
type MetaDataGroupMessage struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	MetaDataGroupName  string                             `protobuf:"bytes,1,opt,name=MetaDataGroupName,proto3" json:"MetaDataGroupName,omitempty"`                                                                                           // The name of the MetaData-Group
	MetaDataInGroupMap map[string]*MetaDataInGroupMessage `protobuf:"bytes,2,rep,name=MetaDataInGroupMap,proto3" json:"MetaDataInGroupMap,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"` // Holding all MetaDataName and their values. The key is the MetaDataName
}

func (x *MetaDataGroupMessage) Reset() {
	*x = MetaDataGroupMessage{}
	if protoimpl.UnsafeEnabled {
		mi := &file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *MetaDataGroupMessage) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MetaDataGroupMessage) ProtoMessage() {}

func (x *MetaDataGroupMessage) ProtoReflect() protoreflect.Message {
	mi := &file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MetaDataGroupMessage.ProtoReflect.Descriptor instead.
func (*MetaDataGroupMessage) Descriptor() ([]byte, []int) {
	return file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_rawDescGZIP(), []int{2}
}

func (x *MetaDataGroupMessage) GetMetaDataGroupName() string {
	if x != nil {
		return x.MetaDataGroupName
	}
	return ""
}

func (x *MetaDataGroupMessage) GetMetaDataInGroupMap() map[string]*MetaDataInGroupMessage {
	if x != nil {
		return x.MetaDataInGroupMap
	}
	return nil
}

// MetaDataInGroupMessage
// Message holding an Item and its values
type MetaDataInGroupMessage struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	MetaDataGroupName                       string                 `protobuf:"bytes,1,opt,name=MetaDataGroupName,proto3" json:"MetaDataGroupName,omitempty"`                                                                                                                                     // The name of the MetaData-Group
	MetaDataName                            string                 `protobuf:"bytes,2,opt,name=MetaDataName,proto3" json:"MetaDataName,omitempty"`                                                                                                                                               // The name of the MetaData-post
	SelectType                              MetaDataSelectTypeEnum `protobuf:"varint,3,opt,name=SelectType,proto3,enum=fenixTestCaseBuilderServerGrpcApi.MetaDataSelectTypeEnum" json:"SelectType,omitempty"`                                                                                    // Is the MetaData-post single- or multi-select
	IsMandatory                             bool                   `protobuf:"varint,4,opt,name=IsMandatory,proto3" json:"IsMandatory,omitempty"`                                                                                                                                                // Is the MetaData-post mandatory or not
	AvailableMetaDataValues                 []string               `protobuf:"bytes,5,rep,name=AvailableMetaDataValues,proto3" json:"AvailableMetaDataValues,omitempty"`                                                                                                                         // The available values for the MetaData-post
	SelectedMetaDataValueForSingleSelect    string                 `protobuf:"bytes,6,opt,name=SelectedMetaDataValueForSingleSelect,proto3" json:"SelectedMetaDataValueForSingleSelect,omitempty"`                                                                                               // The value selected for single select
	SelectedMetaDataValuesForMultiSelect    []string               `protobuf:"bytes,7,rep,name=SelectedMetaDataValuesForMultiSelect,proto3" json:"SelectedMetaDataValuesForMultiSelect,omitempty"`                                                                                               // The values selected for multi select
	SelectedMetaDataValuesForMultiSelectMap map[string]string      `protobuf:"bytes,8,rep,name=SelectedMetaDataValuesForMultiSelectMap,proto3" json:"SelectedMetaDataValuesForMultiSelectMap,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"` // The values selected for multi select, as a map
}

func (x *MetaDataInGroupMessage) Reset() {
	*x = MetaDataInGroupMessage{}
	if protoimpl.UnsafeEnabled {
		mi := &file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *MetaDataInGroupMessage) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MetaDataInGroupMessage) ProtoMessage() {}

func (x *MetaDataInGroupMessage) ProtoReflect() protoreflect.Message {
	mi := &file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MetaDataInGroupMessage.ProtoReflect.Descriptor instead.
func (*MetaDataInGroupMessage) Descriptor() ([]byte, []int) {
	return file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_rawDescGZIP(), []int{3}
}

func (x *MetaDataInGroupMessage) GetMetaDataGroupName() string {
	if x != nil {
		return x.MetaDataGroupName
	}
	return ""
}

func (x *MetaDataInGroupMessage) GetMetaDataName() string {
	if x != nil {
		return x.MetaDataName
	}
	return ""
}

func (x *MetaDataInGroupMessage) GetSelectType() MetaDataSelectTypeEnum {
	if x != nil {
		return x.SelectType
	}
	return MetaDataSelectTypeEnum_MetaDataSelectType_NotSelected
}

func (x *MetaDataInGroupMessage) GetIsMandatory() bool {
	if x != nil {
		return x.IsMandatory
	}
	return false
}

func (x *MetaDataInGroupMessage) GetAvailableMetaDataValues() []string {
	if x != nil {
		return x.AvailableMetaDataValues
	}
	return nil
}

func (x *MetaDataInGroupMessage) GetSelectedMetaDataValueForSingleSelect() string {
	if x != nil {
		return x.SelectedMetaDataValueForSingleSelect
	}
	return ""
}

func (x *MetaDataInGroupMessage) GetSelectedMetaDataValuesForMultiSelect() []string {
	if x != nil {
		return x.SelectedMetaDataValuesForMultiSelect
	}
	return nil
}

func (x *MetaDataInGroupMessage) GetSelectedMetaDataValuesForMultiSelectMap() map[string]string {
	if x != nil {
		return x.SelectedMetaDataValuesForMultiSelectMap
	}
	return nil
}

var File_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto protoreflect.FileDescriptor

var file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_rawDesc = []byte{
	,
}

var (
	file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_rawDescOnce sync.Once
	file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_rawDescData = file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_rawDesc
)

func file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_rawDescGZIP() []byte {
	file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_rawDescOnce.Do(func() {
		file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_rawDescData = protoimpl.X.CompressGZIP(file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_rawDescData)
	})
	return file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_rawDescData
}

var file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_msgTypes = make([]protoimpl.MessageInfo, 8)
var file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_goTypes = []interface{}{
	(MetaDataSelectTypeEnum)(0),                   // 0: fenixTestCaseBuilderServerGrpcApi.MetaDataSelectTypeEnum
	(*UserSpecifiedTestCaseMetaDataMessage)(nil),  // 1: fenixTestCaseBuilderServerGrpcApi.UserSpecifiedTestCaseMetaDataMessage
	(*UserSpecifiedTestSuiteMetaDataMessage)(nil), // 2: fenixTestCaseBuilderServerGrpcApi.UserSpecifiedTestSuiteMetaDataMessage
	(*MetaDataGroupMessage)(nil),                  // 3: fenixTestCaseBuilderServerGrpcApi.MetaDataGroupMessage
	(*MetaDataInGroupMessage)(nil),                // 4: fenixTestCaseBuilderServerGrpcApi.MetaDataInGroupMessage
	nil,                                           // 5: fenixTestCaseBuilderServerGrpcApi.UserSpecifiedTestCaseMetaDataMessage.MetaDataGroupsMapEntry
	nil,                                           // 6: fenixTestCaseBuilderServerGrpcApi.UserSpecifiedTestSuiteMetaDataMessage.MetaDataGroupsMapEntry
	nil,                                           // 7: fenixTestCaseBuilderServerGrpcApi.MetaDataGroupMessage.MetaDataInGroupMapEntry
	nil,                                           // 8: fenixTestCaseBuilderServerGrpcApi.MetaDataInGroupMessage.SelectedMetaDataValuesForMultiSelectMapEntry
}
var file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_depIdxs = []int32{
	5, // 0: fenixTestCaseBuilderServerGrpcApi.UserSpecifiedTestCaseMetaDataMessage.MetaDataGroupsMap:type_name -> fenixTestCaseBuilderServerGrpcApi.UserSpecifiedTestCaseMetaDataMessage.MetaDataGroupsMapEntry
	6, // 1: fenixTestCaseBuilderServerGrpcApi.UserSpecifiedTestSuiteMetaDataMessage.MetaDataGroupsMap:type_name -> fenixTestCaseBuilderServerGrpcApi.UserSpecifiedTestSuiteMetaDataMessage.MetaDataGroupsMapEntry
	7, // 2: fenixTestCaseBuilderServerGrpcApi.MetaDataGroupMessage.MetaDataInGroupMap:type_name -> fenixTestCaseBuilderServerGrpcApi.MetaDataGroupMessage.MetaDataInGroupMapEntry
	0, // 3: fenixTestCaseBuilderServerGrpcApi.MetaDataInGroupMessage.SelectType:type_name -> fenixTestCaseBuilderServerGrpcApi.MetaDataSelectTypeEnum
	8, // 4: fenixTestCaseBuilderServerGrpcApi.MetaDataInGroupMessage.SelectedMetaDataValuesForMultiSelectMap:type_name -> fenixTestCaseBuilderServerGrpcApi.MetaDataInGroupMessage.SelectedMetaDataValuesForMultiSelectMapEntry
	3, // 5: fenixTestCaseBuilderServerGrpcApi.UserSpecifiedTestCaseMetaDataMessage.MetaDataGroupsMapEntry.value:type_name -> fenixTestCaseBuilderServerGrpcApi.MetaDataGroupMessage
	3, // 6: fenixTestCaseBuilderServerGrpcApi.UserSpecifiedTestSuiteMetaDataMessage.MetaDataGroupsMapEntry.value:type_name -> fenixTestCaseBuilderServerGrpcApi.MetaDataGroupMessage
	4, // 7: fenixTestCaseBuilderServerGrpcApi.MetaDataGroupMessage.MetaDataInGroupMapEntry.value:type_name -> fenixTestCaseBuilderServerGrpcApi.MetaDataInGroupMessage
	8, // [8:8] is the sub-list for method output_type
	8, // [8:8] is the sub-list for method input_type
	8, // [8:8] is the sub-list for extension type_name
	8, // [8:8] is the sub-list for extension extendee
	0, // [0:8] is the sub-list for field type_name
}

func init() {
	file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_init()
}
func file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_init() {
	if File_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UserSpecifiedTestCaseMetaDataMessage); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UserSpecifiedTestSuiteMetaDataMessage); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*MetaDataGroupMessage); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_msgTypes[3].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*MetaDataInGroupMessage); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_rawDesc,
			NumEnums:      1,
			NumMessages:   8,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_goTypes,
		DependencyIndexes: file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_depIdxs,
		EnumInfos:         file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_enumTypes,
		MessageInfos:      file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_msgTypes,
	}.Build()
	File_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto = out.File
	file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_rawDesc = nil
	file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_goTypes = nil
	file_FenixTestCaseBuilderServer_fenixTestCaseBuilderServerGrpcApi_fenixTestCaseBuilderServerGrpcApi_MetaDataInTestCaseAndTestSuite_proto_depIdxs = nil
}
